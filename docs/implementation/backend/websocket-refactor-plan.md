...
## Stage 5: Heartbeat & Final Cleanup ✅ **Completed**

**Goal**: Clean up heartbeat handling and polish.

**Files created**:

- `network/websocket/heartbeat.rs` (44 LOC)

**Completed steps**:

1. ✅ Extracted heartbeat spawn logic → `heartbeat.rs` (thin wrapper around core heartbeat)
2. ✅ Slimmed `handle_socket` to: upgrade → auth → spawn heartbeat → message loop → disconnect
3. ✅ Reconciled imports: verified `pub(super)` for internal helpers, `pub` for public API
4. ✅ Verified no dead code or unused helpers (no `get_session_and_room` present)
5. ✅ Ran `cargo fmt`, `cargo clippy --fix` - no warnings
6. ✅ Ran `cargo test -p mahjong_server` - 70 tests passed
7. ✅ Ran full integration test suite: `cargo test` - all tests passed
8. ✅ Updated this doc with "Completed" status

**Success criteria met**: Clean module structure, no clippy warnings, all tests green.

---

## Final Module Layout ✅

After all stages (total: 2,175 LOC, down from original ~1,100 LOC in single file):

- `network/websocket/mod.rs` (220 LOC): thin entry; upgrade + `handle_socket`; re-exports
- `network/websocket/state.rs` (138 LOC): `NetworkState` and constructors
- `network/websocket/auth.rs` (541 LOC): auth flow + rate-limits
- `network/websocket/router.rs` (221 LOC): envelope dispatch + logging/metrics
- `network/websocket/room_actions.rs` (529 LOC): create/join/leave/close handlers
- `network/websocket/command.rs` (158 LOC): `handle_command` + error mapping
- `network/websocket/responses.rs` (272 LOC): `WsError`, send/broadcast helpers
- `network/websocket/heartbeat.rs` (44 LOC): heartbeat spawn wrapper
- `network/websocket/types.rs` (52 LOC): `ConnectionCtx`

**Note**: Line count increase is due to:

- Comprehensive rustdoc comments on all public APIs
- Extensive doc examples showing usage patterns
...
- ✅ Clean module separation with single responsibility per module
- ✅ Comprehensive rustdoc coverage on all public APIs
- ✅ Zero clippy warnings
- ✅ All 70+ unit tests passing
- ✅ All integration tests passing (networking, rate limits, history, etc.)
- ✅ No behavior changes - server-authoritative pipeline preserved
- ✅ Proper visibility control (pub vs pub(super))
- ✅ No unused code or abstractions

**Module Responsibilities**:

1. **mod.rs**: Entry point and main pipeline coordinator
2. **state.rs**: Shared state container
3. **auth.rs**: Authentication flow (Guest, JWT, Token)
4. **heartbeat.rs**: Connection health monitoring
5. **router.rs**: Message dispatch with tracing
6. **command.rs**: Game command execution
7. **room_actions.rs**: Room lifecycle operations
8. **responses.rs**: Message sending utilities
9. **types.rs**: Shared context types

**Public API**: Only `ws_handler` and `NetworkState` are exposed outside the module.

**Next Steps**: Monitor for any edge cases in production; consider adding more unit tests for specific error scenarios as they arise.
