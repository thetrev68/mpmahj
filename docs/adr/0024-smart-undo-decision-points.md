...
1.  **Decision-Point Navigation:** Undo operations will not merely "go back one event" (which might be a system event like `DrawTile`) but will rewind to the most recent *decision point* for the requesting player or the game loop (e.g., before a discard, before a Charleston pass, or before a Call/Pass declaration).
...
3.  **Destructive Truncation:** When a game state is restored and a *new* action is taken, the previous "future" is discarded (history is truncated). We will not support branching history trees at this time.
