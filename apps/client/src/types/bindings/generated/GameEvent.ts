...
export type GameEvent =
  ...
  | { PlayerJoined: { player: Seat; player_id: string; is_bot: boolean } }
  | 'GameStarting'
  | { DiceRolled: { roll: number } }
  | { WallBroken: { position: number } }
  | { TilesDealt: { your_tiles: Array<Tile> } }
  | { CharlestonPhaseChanged: { stage: CharlestonStage } }
  | { PlayerReadyForPass: { player: Seat } }
  | { TilesPassing: { direction: PassDirection } }
  | { TilesReceived: { player: Seat; tiles: Array<Tile> } }
  ...
  | { TileDrawn: { tile: Tile | null; remaining_tiles: number } }
  ...
  | { JokerExchanged: { player: Seat; target_seat: Seat; joker: Tile; replacement: Tile } }
  | { BlankExchanged: { player: Seat } }
  | { MahjongDeclared: { player: Seat } }
  | { HandValidated: { player: Seat; valid: boolean; pattern: string | null } }
  | { GameOver: { winner: Seat | null; result: GameResult } }
