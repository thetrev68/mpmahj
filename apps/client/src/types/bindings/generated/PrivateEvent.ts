// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { IncomingContext } from "./IncomingContext";
import type { ReplacementReason } from "./ReplacementReason";
import type { Seat } from "./Seat";
import type { Tile } from "./Tile";

/**
 * Events routed to a single player or a courtesy pass pair.
 */
export type PrivateEvent = { "TilesDealt": { 
/**
 * Tiles dealt to the recipient.
 */
your_tiles: Array<Tile>, } } | { "TilesPassed": { 
/**
 * Seat that passed tiles.
 */
player: Seat, 
/**
 * Tiles that were passed.
 */
tiles: Array<Tile>, } } | { "TilesReceived": { 
/**
 * Seat receiving tiles.
 */
player: Seat, 
/**
 * Tiles received.
 */
tiles: Array<Tile>, 
/**
 * Optional seat tiles came from (hidden for blind passes).
 */
from: Seat | null, } } | { "TileDrawnPrivate": { 
/**
 * Tile drawn from the wall.
 */
tile: Tile, 
/**
 * Remaining tiles after the draw.
 */
remaining_tiles: number, } } | { "ReplacementDrawn": { 
/**
 * Seat drawing the replacement.
 */
player: Seat, 
/**
 * Replacement tile.
 */
tile: Tile, 
/**
 * Reason for the replacement draw.
 */
reason: ReplacementReason, } } | { "CourtesyPassProposed": { 
/**
 * Seat proposing the count.
 */
player: Seat, 
/**
 * Proposed tile count.
 */
tile_count: number, } } | { "CourtesyPassMismatch": { 
/**
 * Courtesy pair seats.
 */
pair: [Seat, Seat], 
/**
 * Proposed counts in order of the pair tuple.
 */
proposed: [number, number], 
/**
 * Agreed count (smallest value wins).
 */
agreed_count: number, } } | { "CourtesyPairReady": { 
/**
 * Courtesy pair seats.
 */
pair: [Seat, Seat], 
/**
 * Tile count to exchange.
 */
tile_count: number, } } | { "IncomingTilesStaged": { 
/**
 * Seat receiving the tiles.
 */
player: Seat, 
/**
 * Tiles now in the staging area.
 */
tiles: Array<Tile>, 
/**
 * Seat the tiles came from, or `None` when the source is hidden
 * (e.g., blind Charleston stages) or not applicable (Draw).
 */
from: Seat | null, 
/**
 * Why these tiles are arriving.
 */
context: IncomingContext, } };
