 ...
 * ```
 */
export function DrawOverlay({ show, reason, onAcknowledge }: DrawOverlayProps) {
  if (!show) return null;

  const isAbandoned = reason.toLowerCase().includes('dead');
  const title = isAbandoned ? 'GAME ABANDONED' : 'WALL GAME - DRAW';
  ...
  return (
    ...
    >
      ...
      <p className="sr-only">{`Wall exhausted. Game ends in a draw. No winner. ${reason}.`}</p>
      ...
      <div className="bg-gray-900 border-2 border-blue-400 rounded-2xl shadow-2xl px-10 py-8 flex flex-col items-center gap-6 min-w-[340px] max-w-[480px]">
        ...
        <div className="w-full bg-gray-800 rounded-lg px-6 py-4 text-center space-y-2">
          ...
          <p className="text-gray-400 text-sm">Remaining Tiles: 0</p>
        </div>

        {/* Message */}
        <div className="text-center space-y-1">
          <p className="text-gray-300 text-sm">The game ends with no winner.</p>
