...
2. **🟡 Unwrap() Usage** - Replace ~70 unwrap calls with proper error handling
   - **Locations**: `validation.rs`, `charleston.rs`, `call_resolution.rs`
   - **Impact**: Potential panics in production
   - **Fix Time**: 2-4 hours

3. **🟡 Missing Session Cleanup** - Add automatic cleanup task
   - **Location**: `session.rs`
   - **Impact**: Memory leak over time
   - **Fix Time**: 30 minutes

4. **🟡 RwLock Poisoning** - Replace unwrap with expect + monitoring
...
5. **🟡 Unsafe MCTS Pointers** - Replace with arena allocator
   - **Location**: `mcts/engine.rs:166-197`
   - **Impact**: Maintenance burden
   - **Fix Time**: 2-3 hours

6. **🟡 Complex Functions** - Refactor long handlers
...
7. **🟢 Probability Model** - Implement hypergeometric distribution
   - **Location**: `probability.rs:131-157`
   - **Impact**: ~5-15% error (documented trade-off)
   - **Fix Time**: 4-6 hours

8. **🟢 Dependency Updates** - Update to latest stable versions
...
4. **Refactor unwrap() usage** in validation/handlers
5. **Add performance regression tests**
6. **Update dependencies** to latest stable

### 11.3 Long-Term Actions (Next Quarter)

7. **Replace unsafe MCTS pointers** with arena allocator
8. **Implement improved probability model**
9. **Add AI win rate validation tests**
10. **Complete TODO items** in mahjong_terminal
